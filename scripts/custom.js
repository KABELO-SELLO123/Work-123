...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init_template(){
        ...
        //File Upload
        const inputArrays = document.getElementsByClassName('file-data2');
        if(inputArrays.length){
            inputArrays[0].addEventListener('change',prepareUpload,false);
                function prepareUpload(event){
                    if (this.files && this.files[0]) {
                    var img = document.getElementById('image-datas');
                    img.src = URL.createObjectURL(this.files[0]);
                    img.classList.add('mt-1', 'mb-1', 'mx-auto');
                }
                const files = event.target.files;
                const fileName = files[0].name;
                const fileSize = (files[0].size/1000).toFixed(2)+'kb';
                const textBefore = document.getElementsByClassName('upload-file-names')[0].getAttribute('data-text-befores');
                const textAfter = document.getElementsByClassName('upload-file-names')[0].getAttribute('data-text-afters');
                document.getElementsByClassName('upload-file-names')[0].innerHTML =  textBefore + ' ' +  fileName + ' - ' + fileSize + ' - ' + textAfter;
                document.getElementsByClassName('upload-file-names')[0].classList.add('pb-3');
            }
        }
...
function Cellc(){
    var prodName=document.getElementById("provideName").textContent;
    document.getElementById("PayServiceName").textContent=prodName;  
}

function Mtn(){
    var prodName=document.getElementById("provideNamem").textContent;
    document.getElementById("PayServiceName").textContent=prodName;  
}

function Vodacom(){
    var prodName=document.getElementById("provideNamev").textContent;
    document.getElementById("PayServiceName").textContent=prodName;  
}

function Telkom(){
    var prodName=document.getElementById("provideNamet").textContent;
    document.getElementById("PayServiceName").textContent=prodName;  
}
...
function getCardInformationOne(){

    var cardHeader=document.getElementById('card-h').textContent;
    localStorage.setItem('CardTom',cardHeader);
    var cardBottom=document.getElementById('card-b').textContent;
    localStorage.setItem('CardBottom',cardBottom);
    var cardNummber=document.getElementById('card-Number').textContent;
    localStorage.setItem('CardNum',cardNummber);
    var cardAmount=document.getElementById('card-Amount').textContent;
    localStorage.setItem('CardAmount',cardAmount);
    var cardColors=document.getElementById('card-colors').className;
    localStorage.setItem('CardC',cardColors);
    
    window.location.href="view-group.html";

}

function getCardInformationTwo(){

    var cardHeader=document.getElementById('card-hq').textContent;
    localStorage.setItem('CardTom',cardHeader);
    var cardBottom=document.getElementById('card-bq').textContent;
    localStorage.setItem('CardBottom',cardBottom);
    var cardNummber=document.getElementById('card-Numberq').textContent;
    localStorage.setItem('CardNum',cardNummber);
    var cardAmount=document.getElementById('card-Amountq').textContent;
    localStorage.setItem('CardAmount',cardAmount);
    var cardColors=document.getElementById('card-colorsq').className;
    localStorage.setItem('CardC',cardColors);
...
$("#showPage").click(function(){
    ...
    var popup_Window = new bootstrap.Offcanvas($("#menu-pay"));
    popup_Window.show();
